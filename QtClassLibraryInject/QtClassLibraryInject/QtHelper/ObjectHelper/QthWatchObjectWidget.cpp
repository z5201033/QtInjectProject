#include "QthWatchObjectWidget.h"
...
namespace Qth
{
	...
	void WatchObjectWidget::initUI()
	{
		...
		titleWidget->setFixedHeight(25);
		{
			...
			d->_btnFilter->setFixedSize(QthCommon::getContentWidth(d->_btnFilter->font(), d->_btnFilter->text()).width() + 20, 20);

			d->_btnClear = new QPushButton(titleWidget);
			d->_btnClear->setText("Clear");
			d->_btnClear->setFixedSize(QthCommon::getContentWidth(d->_btnClear->font(), d->_btnClear->text()).width() + 20, 20);
			...
			d->_btnHighlight->setFixedSize(QthCommon::getContentWidth(d->_btnHighlight->font(), d->_btnHighlight->text()).width() + 20, 20);
	...
	bool WatchObjectWidget::eventFilter(QObject* watched, QEvent* event)
	{
		do 
		{
			...
			d->_strOut += QthEnumUtils::getEnumName(event->type(), false);
			*d->_debugOut << " " << event;
		} while (false);
