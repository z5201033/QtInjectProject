...
#include "../QthCommon.h"
#include "../ObjectHelper/QthWatchObjectWidget.h"
...
namespace Qth
{
	...
	QWidget* WidgetHelper::createTitle()
	{
		...
		addrLabel->setFixedWidth(QthCommon::getContentWidth(addrLabel->font(), "Address:").width() + 8);
		...
		classLabel->setFixedWidth(QthCommon::getContentWidth(classLabel->font(), "ClassName:").width() + 8);
		...
		findWidgetBtn->setFixedWidth(QthCommon::getContentWidth(findWidgetBtn->font(), "SearchWidget").width() + 20);

		QPushButton* refreshBtn = new QPushButton("RefreshTree", this);
		refreshBtn->setFixedWidth(QthCommon::getContentWidth(refreshBtn->font(), "RefreshTree").width() + 20);
	...
	void WidgetHelper::onFindWidgetBtnPressed()
	{
		...
		do 
		{
			...
			curWidget = QthCommon::convertAddrToWidget(addr);
	...
	void TreeInfoWidgetHelper::initSignalConnect()
	{
		auto highLightFunc = [=](QTreeWidgetItem* item) {
			...
			QWidget* curWidget = QthCommon::convertAddrToWidget(addr);
			if (curWidget)
	...
	void TreeInfoWidgetHelper::showMenu(QTreeWidgetItem* item, int column)
	{
		...
		menu->addAction("watch this widget", [&]() {
			QTimer::singleShot(10, this, [=]() {
				showWatchWidget(m_treeItemToWidget[item]);
			});
		});
...
// 				itemWidget.first->setExpanded(true);
	...
	{
		auto itFind = _watcherList.find(obj);
		if (itFind != _watcherList.end() && itFind.value())
		{
